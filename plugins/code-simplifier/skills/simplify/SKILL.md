...
Run a controlled simplification pass over the smallest relevant code and documentation scope. Preserve external behavior, public interfaces, tests, schemas, config, and generated artifacts unless the user explicitly asks otherwise.
...
Inspect only these files: <paths>.
Do not edit files.
Find unnecessarily nested conditionals, unclear names, over-clever expressions, stale or obvious comments, low-value docstrings, docs that mention local implementation history instead of durable behavior, broad types that can be safely narrowed, and deviations from local project style.
